...
* **Fait — plus de rouge, disposition plus dense (retravail Phase 8, suite)** : demande explicite de l'utilisateur. Le rouge reste strictement borné à `dead`/breach/holdout (I3) — jamais étendu au-delà — mais rendu plus présent dans ce périmètre : `KillCard` d'une stratégie morte hérite d'une bordure gauche épaisse et d'un fond teinté nettement plus marqués ; le leaderboard et la liste de sélection de l'explorateur de combinaisons portent la même bordure gauche rouge sur une ligne `dead` ; `StatCell` gagne un `tone="destructive"`, utilisé pour surligner en rouge plein les probabilités de breach propsim (`P(breach perte journalière)`, `P(breach DD max)`, nouvellement affichées) quand elles dépassent un seuil matériel (≥ 5 %) — ces deux champs existaient déjà dans `PropSimResult` mais n'étaient pas rendus dans la fiche stratégie. `StrategyDetail.tsx` est repassé d'une colonne unique plafonnée à 900px à une disposition en colonnes CSS (`columns-1 xl:columns-2`, chaque section en panneau bordé indépendant, kill card seule en tête hors grille) : hauteur totale de page réduite d'environ 28 % malgré l'ajout des deux nouvelles statistiques. `RiskSurfacePage` place désormais les stats et le tableau complet à côté du graphique plutôt qu'en dessous. Vérifié en exécution réelle : six routes sans erreur console en clair et sombre, `npm run build`/`npm run lint` propres, capture d'écran d'une stratégie morte confirmant le rouge (kill card, breach DD max à 72,5 %) et d'une stratégie saine confirmant qu'aucun signal rouge n'apparaît à tort.
* **Fait — Phase 7 (premier jet)** : `edgelab/papers/` — fiche papier structurée (`PaperSheet`), score de testabilité calculé (`testability.py` : quatre composantes + le facteur post-publication McLean & Pontiff, qui compte double et plafonne à 10 ans), file de triage (`TriageStatus`), stockage SQLite mutable (`PaperRepository` — contrairement au registre d'essais I1 ou au journal holdout I3, la file de triage n'est volontairement pas append-only). Décision de portée actée avec l'utilisateur avant construction : l'utilisateur voulait un pipeline complet papier -> JSON -> hypothèse -> code -> backtest automatique depuis le navigateur ; refusé tel quel après audit — aucune commande CLI `strategy new`/`backtest run` n'existe encore, et le verrou I2 (« pas de backtest sans fiche d'hypothèse validée ») n'est en réalité câblé nulle part dans le moteur, un vrai trou d'invariant trouvé en creusant la demande. Le pipeline construit s'arrête donc à un **brouillon d'hypothèse** (`HypothesisDraft`, réutilisant `HypothesisSheet` telle quelle — même règle de falsifiabilité I2, pas de duplication) : rien n'écrit dans `edgelab/strategies/`, rien ne lance de backtest ; matérialiser un brouillon en stratégie réellement testable reste une action CLI future, distincte, non construite ici. L'extraction PDF et la traduction multilingue ne sont pas non plus construites en interne dans ce premier jet : deux prompts copiables (`prompts.py`, servis à l'identique par le CLI et l'API — un seul contrat JSON) délèguent ce travail à une IA généraliste ; l'utilisateur colle la réponse JSON dans l'UI, validée par les mêmes modèles Pydantic partout. CLI : `edgelab paper add/list/show/hypothesis/hypothesis-prompt/set-status` (`mort` refuse sans motif écrit). API : `GET/POST /api/papers`, `GET /api/papers/{id}`, `GET /api/papers/prompt` (doit être déclaré avant `/api/papers/{paper_id}` dans `main.py` — même forme de route, premier enregistré gagne chez Starlette, testé explicitement), `GET /api/papers/{id}/hypothesis-prompt`, `POST /api/papers/{id}/hypothesis`, `PATCH /api/papers/{id}/status` — tous des déclencheurs d'écriture au sens de `CLAUDE.md` §3 (marshalling HTTP vers `edgelab.papers.PaperRepository`, qui porte seule la validation, jamais de logique dupliquée dans l'API). Frontend : `PapersLibrary.tsx` + `pages/papers/` (intake par copier-coller de prompt + insertion JSON, brouillon d'hypothèse affiché en lecture après rattachement, changement de statut manuel, filtres langue/famille/classe d'actifs, tri par testabilité) ; le rouge (I3/dead, cohérent avec le retravail Phase 8 ci-dessus) s'étend à `mort` pour les papiers — bordure gauche, fond teinté, titre barré, motif de mort affiché. Critère d'acceptation Phase 7 adapté et vérifié : un papier avec `language_source: "zh"` conserve ce code quelle que soit la langue du contenu (la traduction elle-même est déléguée à l'IA d'extraction, documenté comme limite) ; un papier publié en 2023 obtient un score de testabilité strictement inférieur au même papier publié en 2005, toutes choses égales par ailleurs (test synthétique dédié). Vérifié en exécution réelle : CLI (création, hypothèse falsifiable refusée en erreur propre, `mort` sans motif refusé) et API (mêmes cas via `curl` contre un serveur réel) exercés de bout en bout, flux navigateur complet (copier le prompt -> lire le presse-papiers -> coller un JSON -> voir la fiche apparaître -> copier le second prompt -> coller l'hypothèse -> la voir affichée -> refuser un JSON invalide proprement) vérifié par Playwright en clair et sombre, mobile, sans erreur console. 474 tests, 98,85 % de couverture branches côté Python (100 % sur `edgelab/papers/`). `npm run build`/`npm run lint` et `ruff`/`mypy --strict`/`pytest` propres.
* **Fait — données London Strategic Edge + sélection par split (I3 par construction)** : demande explicite de l'utilisateur. Deux trous réels étaient ouverts avant cette session et sont fermés ici. (1) *Aucune commande CLI côté données* : `edgelab/data/` était une bibliothèque complète mais rien ne permettait de faire entrer une barre sans écrire du Python. (2) *Le partitionnement research/validation/holdout n'était branché nulle part* : `DatasetPartition` vivait dans le manifeste, `HoldoutLockbox` comptait bien les accès, mais `DatasetStore.load_bars` renvoyait le dataset entier — holdout compris — et `BacktestEngine` acceptait un `Mapping[str, pl.DataFrame]` brut ; I3 et le refus de la quarantaine tenaient donc par discipline, pas par construction.
  * `data/lse.py` : fournisseur London Strategic Edge, adossé au SDK officiel `lse-data` (dépendance **optionnelle**, extra `lse`, import paresseux) plutôt que réimplémenté. Le protocole `LseClient` est la seule frontière réseau et le seul point injecté dans les tests — même pattern que `Fetcher` pour Dukascopy. Pagination automatique sous le plafond de **5 000 lignes par appel** du vault (sans elle, « deux ans de M1 » renvoie silencieusement une série tronquée qui passe pour complète), garde-fou contre une page répétée à l'infini, budget d'appels borné. `normalise_candles` est pure et accepte plusieurs alias de colonnes et d'encodages d'horodatage. `ingest_lse_candles` (API JSON) et `ingest_lse_export` (job Parquet côté serveur) traversent le même `ingest_bars`, donc le même contrôle d'intégrité et la même quarantaine que toute autre source ; la `frequency` du contrôle vient du `timeframe` demandé, jamais des données reçues.
  * `data/selection.py` : `DataSplit`, `SplitBounds`, `DatasetSelection`, `load_split`, `load_holdout`, `describe_splits`. `DatasetSelection` est le **seul** type que `BacktestEngine` accepte désormais, et il ne peut pas exister sans que le manifeste ait passé `ensure_backtest_ready`, que les barres tombent dans les bornes du split annoncé, et qu'un split `holdout` porte un `HoldoutAccessRecord` — que seul `load_holdout` produit. `load_split` refuse le holdout sans exception. `describe_splits` compte les barres de chaque split sans consommer d'accès (compter n'est pas regarder). `load_holdout` journalise *avant* de lire : le compteur mesure la décision de regarder, pas le succès de la lecture.
  * `backtest/engine.py` : signature changée en `Mapping[str, DatasetSelection]`, refus explicite de mélanger deux splits dans un même run, `dataset_ids`/`dataset_hashes` exposés pour la lignée d'un essai (I1). Les 29 tests Phase 3 ont été migrés (fixtures `wrap_selection`/`make_selection`) sans changer aucun comportement du moteur.
  * CLI : `edgelab data catalog | download | list | show | holdout`. Les bornes de partition de `download` n'ont volontairement **aucun défaut** — où s'arrête la recherche et où commence le holdout est la décision qui protège l'utilisateur de lui-même. Un dataset mis en quarantaine sort en code 2 (téléchargé mais inutilisable), distinct du code 1 (échec).
  * API : `/api/datasets`, `/api/datasets/{id}`, `/api/instruments` en lecture ; `/api/provider/catalog`, `/api/provider/download`, `/api/datasets/holdout` en déclencheurs de job délégant intégralement à `edgelab.data` (l'API ne réimplémente pas I3, elle appelle `load_holdout`).
  * Web : nouvelle vue « Données » (`DataLibrary.tsx` + `pages/data/`) — recherche dans le catalogue LSE, formulaire de téléchargement en trois étapes explicites, suggestion 60/20/20 proposée dans des champs modifiables (jamais appliquée silencieusement), et liste des datasets locaux avec `SplitBar` (le holdout est hachuré et libellé « scellé »). La quarantaine hérite du traitement rouge existant (bordure gauche, fond teinté) et n'est jamais filtrée. L'ouverture du holdout est délibérément inconfortable : pas de bouton en un clic, un champ de raison obligatoire, puis le compteur et le drapeau rouge affichés après coup.
  * Vérifié en exécution réelle : CLI de bout en bout sur deux datasets réellement ingérés (`data list`/`show`/`holdout`, drapeau rouge au 3ᵉ accès observé) ; API réelle interrogée au `curl` ; les six + une routes rendues par Playwright en clair, sombre et mobile sans erreur console, flux holdout complet exercé dans le navigateur (formulaire -> compteur -> drapeau rouge), catalogue sans clé rendant le message actionnable nommant `LSE_API_KEY`. Le protocole `LseClient` est confronté au vrai `lse.LSE` sous `mypy --strict` (signatures identiques). 549 tests, 98,18 % de couverture branches, `ruff`/`mypy --strict`/`npm run build`/`npm run lint` propres.
  * **Limite documentée, à lever en priorité** : aucune clé API n'était disponible, donc le contrat exact du vault (noms de colonnes, format d'horodatage, comportement serveur de la pagination) est déduit de la lecture du SDK et **non vérifié contre un serveur réel**. Le premier téléchargement avec une vraie clé est la vérification manquante ; `normalise_candles` lève en nommant les colonnes reçues plutôt que d'inventer une barre si le schéma diffère.
* **Fait — réglages et stockage local des clés API** : demande explicite de l'utilisateur (« je remplis où la clé api ? »). Avant, la seule voie était `export LSE_API_KEY=...` avant de lancer le serveur, perdu à chaque session. `edgelab/settings.py` : `CredentialStore` écrit un fichier `.edgelab/credentials.env` en **0600** (créé directement restreint via `os.open`, pas ouvert puis durci — sinon il existe une fenêtre où la clé est lisible par toute la machine), format `.env` éditable à la main et sourçable dans un shell. Précédence : **la variable d'environnement gagne sur le fichier**, parce que l'inverse rendrait un déploiement imprévisible (on croit configurer par l'environnement, un fichier oublié décide à sa place) ; `describe()` expose la provenance effective pour que ce ne soit jamais une surprise. `describe()` ne renvoie que l'existence, la provenance et les quatre derniers caractères — jamais la valeur ; une clé de moins de huit caractères ne montre rien du tout, parce qu'en révéler quatre coupe l'espace de recherche de moitié. `resolve()` est la seule fonction qui renvoie un secret, et n'est appelée que par le code qui construit un client fournisseur. CLI : `edgelab config list | set | unset` (`set` demande la valeur en saisie masquée quand elle n'est pas fournie, pour ne pas la laisser dans l'historique du shell, et avertit si une variable d'environnement gardera la priorité). API : `GET /api/settings`, `PUT`/`DELETE /api/settings/{env_var}` — **aucune route ne renvoie de secret et aucune ne le pourrait**, le schéma de réponse ne porte pas la valeur ; les deux routes d'écriture refusent en 403 un appel venant d'ailleurs que de la boucle locale, l'API n'ayant aucune authentification. Web : page « Réglages » (champs `type=password`, état masqué, provenance affichée, ajout d'une clé libre pour un fournisseur pas encore branché — marquée « lue par aucun module » plutôt que de laisser croire qu'elle fait quelque chose). Un vrai bug a été trouvé en vérification réelle : avec une clé bidon enregistrée, le vault répond 401 et l'erreur du SDK remontait en traceback brut ; corrigé par un traducteur d'erreurs à la frontière réseau (`_TranslatingClient`), toute erreur fournisseur est désormais une `LseDataError`. Vérifié en exécution réelle : CLI (fichier bien en 600, aucune clé jamais imprimée en clair), API au `curl`, page Réglages par Playwright en clair/sombre/mobile (champ masqué, clé absente du DOM, empreinte visible, clé non branchée signalée), et la chaîne complète prouvée de bout en bout — clé posée par l'UI -> `resolve` -> SDK -> vault répondant `401 invalid api key`. 590 tests, 98,19 % de couverture branches (100 % sur `edgelab/settings.py`).
* **Prochaine session** : premier téléchargement réel avec une clé `LSE_API_KEY` renseignée dans la page Réglages (lever la limite ci-dessus) ; puis le chemin CLI manquant identifié en Phase 7 (`edgelab strategy new`/`backtest run` + câblage effectif du verrou I2 dans le moteur de backtest), qui peut maintenant s'appuyer sur `DatasetSelection` pour sa lignée ; ou le code-splitting du bundle web (> 500 kB minifié). Toutes les phases listées en §5 sont livrées.
