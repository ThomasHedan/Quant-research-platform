...
**Complément — réglages.** `/api/settings` renvoie l'état de chaque emplacement de clé : existence, provenance effective (variable d'environnement ou fichier local) et derniers caractères. **Aucune route ne renvoie jamais un secret en clair, et il n'en existe pas qui le puisse** — le schéma de réponse ne porte pas la valeur. `PUT`/`DELETE` écrivent via `edgelab.settings.CredentialStore`, qui porte seul la validation et les permissions du fichier. Ces deux routes vérifient en plus que l'appel vient de la boucle locale : l'API n'a aucune authentification, ce qui est acceptable pour un outil personnel servi sur `127.0.0.1` et inacceptable dès qu'il écoute ailleurs — le garde-fou ne fait donc pas confiance à l'utilisateur pour ne jamais lancer `--host 0.0.0.0`.
